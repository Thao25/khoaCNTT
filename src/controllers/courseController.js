...
const createCourse = async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      data: { ...course.toObject(), image: fullImageUrl },
    });
  } catch (error) {
    console.log(error);
      ...
      ? course.documents.map(
          (doc) =>
            `${req.protocol}://${req.get(
              "host"
            )}/uploads/courses/documents/${doc}`
        )
      : [];
    return {
      ...course.toObject(),
      image: fullImageUrl,
      documents: fullDocumentUrls,
    };
  });

  res
    .status(200)
    .json({ message: "Danh sách khóa học", data: coursesWithUrls });
};
...
const updateCourse = async (req, res) => {
  ...
  res.status(200).json({
    ...
    data: { ...updatedCourse.toObject(), image: fullImageUrl },
  });
};
...
const addDocuments = async (req, res) => {
  try {
    ...
    const documentUrls = updatedCourse.documents.map(
      (doc) =>
        `${req.protocol}://${req.get("host")}/uploads/courses/documents/${doc}`
    );
    ...
    res
      .status(500)
      .json({ message: "Lỗi trong quá trình thêm tài liệu vào khóa học" });
  }
};
